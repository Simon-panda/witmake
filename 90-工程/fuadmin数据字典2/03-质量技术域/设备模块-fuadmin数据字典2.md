...
> 域: 03-质量技术域 | 表数: 5 | 字段: 97 | 代码锚定: 92(95%) | 生成: 2026-07-24 | 上游: [[fuadmin数据字典2总览]] | 旧版: [[90-工程/fuadmin数据字典/03-质量技术域/设备模块-fuadmin数据字典]]
...
| 表 | 定义 | 行数(估) | 锚点 |
|---|---|---|---|
| `generator_equipment_inspection` | 设备点检 | 12993 | 💻 |
| `generator_equipment_maintenance_records` | 设备保养记录 | 1377 | 💻 |
| `generator_equipment_repair` | 设备报修(新) | 5896 | 💻 |
| `generator_equipment_repair_report` | 设备报修 | 718 | 💻 |
...
**定义**：设备点检 ｜ **代码**：`generator/equipment_inspection/model.py` ｜ **行数(估)**：12993

| 字段 | 类型 | 可空 | 键 | 语义 | 证据 | 关联 |
|---|---|---|---|---|---|---|
| `id` | bigint | N | PRI | Id（界面:点检单号） | ✅📦🖥️ |  |
...
**定义**：设备保养记录 ｜ **代码**：`generator/equipment_maintenance_records/model.py` ｜ **行数(估)**：1377

| 字段 | 类型 | 可空 | 键 | 语义 | 证据 | 关联 |
|---|---|---|---|---|---|---|
| `id` | bigint | N | PRI | Id（界面:保养单号） | ✅📦🖥️ |  |
| `remark` | varchar(255) | Y | - | 描述（界面:备注） | ✅📦🖥️ |  |
| `modifier` | varchar(255) | Y | - | 修改人 | ✅📦 |  |
| `belong_dept` | int | Y | - | 数据归属部门（界面:所属部门） | ✅📦🖥️ |  |
| `update_datetime` | datetime(6) | Y | - | 修改时间（界面:未结单） | ✅📦🖥️ |  |
...
| `creator_id` | bigint | Y | MUL | 🔍待补充 | 🔍 |  |
| `last_maintenance_date` | date | Y | - | 上次保养日期（界面:上次日期） | ✅🖥️ |  |
| `maintenance_type` | varchar(255) | Y | - | 保养类型 | ✅ |  |
| `states` | int | Y | - | 状态；待保养1/已保养2 | ✅💬 |  |

### generator_equipment_repair
**定义**：设备报修(新) ｜ **代码**：`generator/equipment_repair/model.py` ｜ **行数(估)**：5896
...
| `create_datetime` | datetime(6) | Y | - | 创建时间 | ✅📦 |  |
| `sort` | int | Y | - | 显示排序 | ✅📦 |  |
| `warranty_number` | varchar(40) | Y | - | 报修单号；BXDH20250312075974 | ✅💬 |  |
| `evaluate` | varchar(255) | Y | - | 维修评价 | ✅ |  |
| `problem_description` | varchar(255) | Y | - | 问题描述 | ✅ |  |
| `cc_personnel` | varchar(255) | Y | - | 抄送人员；['22383 袁恒亮', '13028 徐红雷', '14065 谢红桥'] | ✅💬 |  |
| `direct_time` | datetime(6) | Y | - | 接单日期；2025-03-12 08:22:00 | ✅💬 |  |
| `repair_time` | datetime(6) | Y | - | 报修日期；2025-03-12 07:59:00 | ✅💬 |  |
| `rate_time` | datetime(6) | Y | - | 评分时间；2025-03-12 09:00:00（界面:评分日期） | ✅💬🖥️ |  |
| `malfunction` | tinyint(1) | Y | - | 有无故障；有故障 True/无故障 False 有故障时需要填写故障明细 | ✅💬 |  |
| `repetition` | tinyint(1) | Y | - | 需要配件；需要配件 True/不需要 False 需要配件时需要填写配件清单 | ✅💬 |  |
| `stop_time` | varchar(20) | Y | - | 暂停时长 | ✅ |  |
| `maintain_time` | varchar(20) | Y | - | 维修时长；21:15:05 | ✅💬 |  |
| `order_time` | varchar(20) | Y | - | 接单时长；0:01:23 | ✅💬 |  |
| `inspection_states` | int | Y | - | 订单状态；待接单1/待检修2/检修中3/已检修4/已评分5/已完成6 | ✅💬 |  |
| `difficulty_level` | int | Y | - | 难度等级；1-10 | ✅💬 |  |
| `emergency_level` | int | Y | - | 紧急等级；一级1 二级2 三级3 四级4 1级最高 | ✅💬 |  |
| `is_active` | int | Y | - | 状态；2启用 1暂停 0注销 | ✅💬 |  |
| `rate` | int | Y | - | 评分；0-5分 | ✅💬 |  |
...
| `repair_applicant_id` | bigint | Y | MUL | 报修人；→system_users（界面:全部） | ✅🔗🖥️ | →system_users |
| `img` | json | Y | - | 相关照片；["/static/20250315/20250315101504580989_IMG_20250315_101044.jpg", "/static/20250315/20250315101512386944_b746847c2eed26f505b4a0dda50f8fc9_compress.jpg", "/static/20250315/20250315101523495378_73bc9d0140fb8c9a7c2e797d4902fcca_compress.jpg"] | ✅💬 |  |
| `message` | json | Y | - | 留言 | ✅ |  |
| `cnc_personnel` | varchar(40) | Y | - | 机床操作人；10123 倪祥雄（界面:机床使用人） | ✅💬🖥️ |  |
| `fault_class` | varchar(40) | Y | - | 故障分类 | ✅ |  |
| `zl_personnel` | varchar(255) | Y | - | 质量人员；['15026 夏玉强'] | ✅💬 |  |
| `after_img` | json | Y | - | 维修后照片；["/static/20250314/20250314164754456667_20d6891edb526963814d1f14eed1d6c6_compress.jpg"] | ✅💬 |  |

### generator_equipment_repair_report
**定义**：设备报修 ｜ **代码**：`generator/equipment_repair_report/model.py` ｜ **行数(估)**：718

| 字段 | 类型 | 可空 | 键 | 语义 | 证据 | 关联 |
|---|---|---|---|---|---|---|
| `id` | bigint | N | PRI | Id（界面:报修单号） | ✅📦🖥️ |  |
| `remark` | varchar(255) | Y | - | 描述（界面:备注） | ✅📦🖥️ |  |
| `modifier` | varchar(255) | Y | - | 修改人 | ✅📦 |  |
| `belong_dept` | int | Y | - | 数据归属部门（界面:所属部门） | ✅📦🖥️ |  |
| `update_datetime` | datetime(6) | Y | - | 修改时间（界面:未结单） | ✅📦🖥️ |  |
...
| `repair_completion_date` | datetime(6) | Y | - | 维修完成日期（界面:无故障） | ✅🖥️ |  |
...
| `history` | varchar(1000) | Y | - | 记录（界面:历史记录） | ✅🖥️ |  |
...
**定义**：M2M中间表: generator_equipment_repair_report ↔ system_users（抄送人员） ｜ **类型**：🔗M2M中间表 ｜ **行数(估)**：1934

| 字段 | 类型 | 可空 | 键 | 语义 | 证据 | 关联 |
|---|---|---|---|---|---|---|
| `id` | bigint | N | PRI | 中间表主键（界面:列表权限） | 🔗🖥️ |  |
| `equipmentrepairreport_id` | bigint | N | MUL | 🔍待补充 | 🔍 |  |
