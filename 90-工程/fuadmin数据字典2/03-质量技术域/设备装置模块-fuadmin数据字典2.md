...
> 域: 03-质量技术域 | 表数: 4 | 字段: 66 | 代码锚定: 62(94%) | 生成: 2026-07-24 | 上游: [[fuadmin数据字典2总览]] | 旧版: [[90-工程/fuadmin数据字典/03-质量技术域/设备装置模块-fuadmin数据字典]]
...
| 表 | 定义 | 行数(估) | 锚点 |
|---|---|---|---|
| `generator_device_cutting_fluid_check` | 设备切削液浓度巡检 | 344 | 💻 |
| `generator_device_eff` | 设备效率分析表 | 2 | 💻 |
| `generator_devices` | 设备表 | 543 | 💻 |
| `generator_devices_log` | 设备运行记录表 | 11 | 💻 |

---

### generator_device_cutting_fluid_check
**定义**：设备切削液浓度巡检 ｜ **代码**：`generator/device_cutting_fluid_check/model.py` ｜ **行数(估)**：344
...
**定义**：设备效率分析表 ｜ **代码**：`generator/device_eff/model.py` ｜ **行数(估)**：2
...
### generator_devices
**定义**：设备表 ｜ **代码**：`generator/devices/model.py` ｜ **行数(估)**：543

| 字段 | 类型 | 可空 | 键 | 语义 | 证据 | 关联 |
|---|---|---|---|---|---|---|
| `id` | bigint | N | PRI | Id（界面:部门） | ✅📦🖥️ | →generator_equipment_repair |
| `remark` | varchar(255) | Y | - | 描述（界面:备注） | ✅📦🖥️ |  |
| `modifier` | varchar(255) | Y | - | 修改人 | ✅📦 |  |
| `belong_dept` | int | Y | - | 数据归属部门（界面:所属部门） | ✅📦🖥️ |  |
| `update_datetime` | datetime(6) | Y | - | 修改时间（界面:未结单） | ✅📦🖥️ |  |
| `create_datetime` | datetime(6) | Y | - | 创建时间 | ✅📦 |  |
| `sort` | int | Y | - | 显示排序 | ✅📦 |  |
| `status` | varchar(255) | Y | - | 设备状态；运行 停机 报警 | ✅💬 |  |
| `last_maintenance` | date | Y | - | 最后维护日期 | ✅ |  |
| `ownership` | varchar(255) | Y | - | 设备归属 | ✅ |  |
| `manager` | varchar(255) | Y | - | 设备负责人 | ✅ |  |
| `production_line` | varchar(255) | Y | - | 产线；工作代号表关联name的数据 能存储多条产线因为一台设备可以生产多个产品或者同种不一样的型号的产品 | ✅💬 |  |
...
| `internal_code` | varchar(255) | Y | MUL | 厂内编号；唯一ID | ✅💬 | →generator_newscheduling(推断) |
...
**定义**：设备运行记录表 ｜ **代码**：`generator/devices_log/model.py` ｜ **行数(估)**：11
