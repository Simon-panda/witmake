...
> 域: 02-生产铸造域 | 表数: 6 | 字段: 88 | 代码锚定: 82(93%) | 生成: 2026-07-24 | 上游: [[fuadmin数据字典2总览]] | 旧版: [[90-工程/fuadmin数据字典/02-生产铸造域/生产计划模块-fuadmin数据字典]]
...
| `sort` | int | Y | - | 显示排序 | ✅📦 |  |
| `period_type` | varchar(10) | N | - | 计划周期；枚举[year=年计划,month=月计划,day=日计划]（界面:类型） | ✅🔢🖥️ |  |
...
| `operator` | varchar(50) | N | - | 操作人（界面:操作员） | ✅🖥️ |  |
...
| `scrap_stock` | int | N | - | 库存报废品数（界面:报废品库存） | ✅🖥️ |  |
| `blank_stock` | int | N | - | 毛坯库存 | ✅ |  |
| `in_process_qty` | int | N | - | 生产流转中(待入+品质过程数) | ✅ |  |
| `creator_id` | bigint | Y | MUL | [推断]创建人（CoreModel审计字段，存用户标识） | 🔍 |  |
| `product_config_id` | bigint | N | MUL | 产品；→generator_plan_product_config | ✅🔗 | →generator_plan_product_config |
| `repair_stock` | int | N | - | 返修库存 | ✅ |  |
| `customer_stock` | int | N | - | 客户方库存；已提前发货存放在客户仓库、尚未被客户生产消耗的数量（界面:客户库存） | ✅🖥️ |  |
...
| `contact_person` | varchar(50) | Y | - | 客户端对接人（界面:客户对接人） | ✅🖥️ |  |
| `is_active` | tinyint(1) | N | - | 是否启用（界面:状态） | ✅🖥️ |  |
| `creator_id` | bigint | Y | MUL | [推断]创建人（CoreModel审计字段，存用户标识） | 🔍 |  |
| `site` | varchar(20) | N | - | 站点；治通 / 广汇 / 泰峰（界面:地点） | ✅🖥️ |  |
