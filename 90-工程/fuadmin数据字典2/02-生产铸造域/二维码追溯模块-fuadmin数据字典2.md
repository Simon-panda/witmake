...
> 域: 02-生产铸造域 | 表数: 13 | 字段: 215 | 代码锚定: 201(93%) | 生成: 2026-07-24 | 上游: [[fuadmin数据字典2总览]] | 旧版: [[90-工程/fuadmin数据字典/02-生产铸造域/二维码追溯模块-fuadmin数据字典]]
...
| `generator_qrcode_in` | 扫码入库 | 0 | 💻 |
| `generator_qrcode_out` | 扫码出库 | 0 | 💻 |
| `generator_qrcode_sendout` | 扫码发货 | 39588 | 💻 |
| `generator_registration_qrcode` | 登记二维码 | 7 | 💻 |
| `generator_tracking_transfer` | 跟踪单流转记录 | 0 | 💻 |
| `generator_tracking_waste_record` | 不良品记录 | 0 | 💻 |
| `track_route` | 工艺路线 | 0 | 💻 |
| `track_route_step` | 工艺路线步骤 | 0 | 💻 |
| `track_station` | 跟踪站点 | 0 | 💻 |
| `track_station_job_code_map` | 站点-计价代码映射 | 0 | 💻 |
...
| `is_active` | tinyint(1) | Y | - | 是否激活（界面:状态） | ✅🖥️ |  |
| `update_datetime` | datetime(6) | Y | - | 修改时间（界面:未结单） | ✅🖥️ |  |
| `create_datetime` | datetime(6) | Y | - | 创建时间 | ✅ |  |
| `system_user` | tinyint(1) | Y | - | 是否系统用户：True内部用户 False外部用户 | 💬⚖️ |  |
| `box_code` | varchar(100) | Y | - | 箱体二维码（该字段未启用）（界面:箱码） | ✅⚖️🖥️ |  |
| `img` | json | Y | - | [推断]现场图片（异常/换码佐证图）（界面:图片信息） | 🔍🖥️ |  |
| `operate_state` | varchar(50) | Y | - | 操作名称：人工确认/隔离/退箱/人工修复/物流发货 | 💬⚖️ |  |
| `creator_id` | bigint | Y | MUL | 创建人；→settings.AUTH_USER_MODEL | ✅🔗 | →settings.AUTH_USER_MODEL |
...
| `temporary_state` | int | Y | - | 临时状态：0不合格 1合格 2隔离 3报废（界面:返修状态） | 💬⚖️🖥️ |  |
...
| `create_datetime` | datetime(6) | Y | - | 创建时间 | ✅📦 |  |
| `sort` | int | Y | - | 显示排序 | ✅📦 |  |
| `image` | json | Y | - | 图片：现场图片（JSON列表）（界面:相关图片） | ✅⚖️🖥️ |  |
| `cc` | json | Y | - | 抄送 | ✅ |  |
...
| `flag` | tinyint(1) | Y | - | 是否验证：是否需要验证产品二维码（界面:是否验证零件） | ✅⚖️🖥️ |  |
...
### generator_qrcode_in
**定义**：扫码入库 ｜ **代码**：`generator/qrcode_in/model.py` ｜ **行数(估)**：0
...
**定义**：扫码出库 ｜ **代码**：`generator/qrcode_out/model.py` ｜ **行数(估)**：0

| 字段 | 类型 | 可空 | 键 | 语义 | 证据 | 关联 |
|---|---|---|---|---|---|---|
| `id` | bigint | N | PRI | Id（界面:出库时间） | ✅📦🖥️ |  |
| `remark` | varchar(255) | Y | - | 描述（界面:备注） | ✅📦🖥️ |  |
| `modifier` | varchar(255) | Y | - | 修改人 | ✅📦 |  |
| `belong_dept` | int | Y | - | 数据归属部门（界面:所属部门） | ✅📦🖥️ |  |
| `update_datetime` | datetime(6) | Y | - | 修改时间（界面:未结单） | ✅📦🖥️ |  |
...
| `create_datetime` | datetime(6) | Y | - | 创建时间 | ✅📦 |  |
| `sort` | int | Y | - | 显示排序 | ✅📦 |  |
| `user` | varchar(20) | N | - | 扫码人；执行出库操作的人员（界面:员工） | ✅🖥️ |  |
| `box_code` | varchar(255) | N | - | 箱码；产品箱码 | ✅ |  |
| `product_name` | varchar(20) | N | - | 产品名称 | ✅ |  |
| `bar_code` | varchar(30) | N | - | 物料条形码：产品物料编码（非件码） | ✅⚖️ |  |
| `number` | int | N | - | 实发数量：每箱实发数，交付件数=SUM(number)（界面:库存数量） | ✅⚖️🖥️ |  |
| `type` | int | N | - | 扫码状态：0错误 1正常 2退货 3隐藏；默认查0/1/2（界面:问题分类） | 💬⚖️🖥️ |  |
| `creator_id` | bigint | Y | MUL | [推断]创建人（CoreModel审计字段，存用户标识） | 🔍 |  |
...
| `check_number` | int | Y | - | 整箱数量：每箱的外检合格数量（界面:外检数量） | ✅⚖️🖥️ |  |
...
**定义**：跟踪单流转记录 ｜ **代码**：`generator/production_tracking/model.py` ｜ **行数(估)**：0
...
**定义**：不良品记录 ｜ **代码**：`generator/production_tracking/model.py` ｜ **行数(估)**：0
...
### track_route
**定义**：工艺路线 ｜ **代码**：`generator/production_tracking/model.py` ｜ **行数(估)**：0

| 字段 | 类型 | 可空 | 键 | 语义 | 证据 | 关联 |
|---|---|---|---|---|---|---|
| `id` | bigint | N | PRI | Id（界面:步骤序号） | ✅📦🖥️ |  |
| `remark` | varchar(255) | Y | - | 描述（界面:备注） | ✅📦🖥️ |  |
| `modifier` | varchar(255) | Y | - | 修改人 | ✅📦 |  |
| `belong_dept` | int | Y | - | 数据归属部门（界面:所属部门） | ✅📦🖥️ |  |
| `update_datetime` | datetime(6) | Y | - | 修改时间（界面:未结单） | ✅📦🖥️ |  |
...
### track_route_step
**定义**：工艺路线步骤 ｜ **代码**：`generator/production_tracking/model.py` ｜ **行数(估)**：0
...
### track_station
**定义**：跟踪站点 ｜ **代码**：`generator/production_tracking/model.py` ｜ **行数(估)**：0

| 字段 | 类型 | 可空 | 键 | 语义 | 证据 | 关联 |
|---|---|---|---|---|---|---|
| `id` | bigint | N | PRI | Id（界面:站点名称） | ✅📦🖥️ |  |
| `remark` | varchar(255) | Y | - | 描述（界面:备注） | ✅📦🖥️ |  |
| `modifier` | varchar(255) | Y | - | 修改人 | ✅📦 |  |
| `belong_dept` | int | Y | - | 数据归属部门（界面:所属部门） | ✅📦🖥️ |  |
| `update_datetime` | datetime(6) | Y | - | 修改时间（界面:未结单） | ✅📦🖥️ |  |
...
**定义**：站点-计价代码映射 ｜ **代码**：`generator/production_tracking/model.py` ｜ **行数(估)**：0
